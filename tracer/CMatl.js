...
function CMatl(matlType) {
  this.K_emit = vec4.create();		// JS arrays that hold 4 reflectance values:
  ...
  if ( matlType && matlType >=0 && matlType < MATL_MAX)	{
    this.setMatl(matlType);			// YES! set the reflectance values (K_xx)
  }
  return this;
}
    ...
    case MATL_MIRROR:
      ...
      break;
		default:
			...
			this.K_emit = vec4.fromValues(0.5, 0.0, 0.0, 1.0); // DEFAULT: ugly RED emissive light only
			this.K_ambi = vec4.fromValues(0.0, 0.0, 0.0, 1.0); // r,g,b,alpha  ambient reflectance
			this.K_diff = vec4.fromValues(0.0, 0.0, 0.0, 1.0); //              diffuse reflectance
			this.K_spec = vec4.fromValues(0.0, 0.0, 0.0, 1.0); //              specular reflectance
